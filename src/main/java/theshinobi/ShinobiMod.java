...
public class ShinobiMod
  ...
  // This will be called by ModTheSpire because of the @SpireInitializer annotation at the top of
  // the class.
  ...
  public ShinobiMod() {
    BaseMod.subscribe(
        this); // This will make BaseMod trigger all the subscribers at their appropriate times.
  ...
  public void receivePostInitialize() {
    ...
    BaseMod.registerModBadge(
        badgeTexture, info.Name, GeneralUtils.arrToString(info.Authors), info.Description, null);
  }
  ...
  public void receiveEditStrings() {
    ...
    */
    loadLocalization(
        defaultLanguage); // no exception catching for default localization; you better have at
  ...
  private void loadLocalization(String lang) {
    // While this does load every type of localization, most of these files are just outlines so
    // that you can see how they're formatted.
    ...
    BaseMod.loadCustomStringsFile(
        CharacterStrings.class, localizationPath(lang, "CharacterStrings.json"));
    ...
    BaseMod.loadCustomStringsFile(
        PotionStrings.class, localizationPath(lang, "PotionStrings.json"));
  ...
  public void receiveEditKeywords() {
    ...
    String json =
        Gdx.files
            .internal(localizationPath(defaultLanguage, "Keywords.json"))
    ...
    if (!defaultLanguage.equals(getLangString())) {
      try {
        json =
            Gdx.files
                .internal(localizationPath(getLangString(), "Keywords.json"))
                .readString(String.valueOf(StandardCharsets.UTF_8));
  ...
  // These methods are used to generate the correct filepaths to various parts of the resources
  // folder.
  ...
  private static String checkResourcesPath() {
    String name =
        ShinobiMod.class
            .getName(); // getPackage can be iffy with patching, so class name is used instead.
    ...
    if (!resources.exists()) {
      throw new RuntimeException(
          "\n\tFailed to find resources folder; expected it to be named \""
              + name
              + "\"."
              + " Either make sure the folder under resources has the same name as your mod's package, or change the line\n"
              + "\t\"private static final String resourcesFolder = checkResourcesPath();\"\n"
              + "\tat the top of the "
              + ShinobiMod.class.getSimpleName()
              + " java file.");
    }
    if (!resources.child("images").exists()) {
      throw new RuntimeException(
          "\n\tFailed to find the 'images' folder in the mod's 'resources/"
              + name
    ...
    if (!resources.child("localization").exists()) {
      throw new RuntimeException(
          "\n\tFailed to find the 'localization' folder in the mod's 'resources/"
              + name
              + "' folder; Make sure the "
        ...
        Arrays.stream(Loader.MODINFOS)
            .filter(
                (modInfo) -> {
                  ...
                  return initializers.contains(ShinobiMod.class.getName());
                })
            .findFirst();
    if (infos.isPresent()) {
      info = infos.get();
