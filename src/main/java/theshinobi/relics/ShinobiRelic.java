package theshinobi.relics;

import static theshinobi.ShinobiMod.relicPath;
...
public abstract class ShinobiRelic extends CustomRelic {
  ...
  public ShinobiRelic(String id, RelicTier tier, LandingSound sfx) {
    this(id, GeneralUtils.removePrefix(id), tier, sfx);
  }

  // To use a basegame relic image, just pass in the imagename used by a basegame relic instead of
  // the ID.
  // eg. "calendar.png"
  public ShinobiRelic(String id, String imageName, RelicTier tier, LandingSound sfx) {
  ...
  protected void loadTexture() {
    ...
    if (img != null) {
      ...
      if (outlineImg == null) outlineImg = img;
    } else {
  ...
  /**
   * Checks whether relic has localization set up correctly and gives a more accurate error message
   * if it does not
   *
  ...
  private static String testStrings(String ID) {
    ...
    if (text == null) {
      throw new RuntimeException(
          "The \""
              + ID
              + "\" relic does not have associated text. Make sure "
              + "there's no issue with the RelicStrings.json file, and that the ID in the json file matches the "
              + "relic's ID. It should look like \"${modID}:"
              + GeneralUtils.removePrefix(ID)
              + "\".");
    }
    return ID;
  }
