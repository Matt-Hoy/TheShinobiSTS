package theshinobi.cards;

import static theshinobi.util.GeneralUtils.removePrefix;
import static theshinobi.util.TextureLoader.getCardTextureString;
...
public abstract class ShinobiCard extends CustomCard {
  ...
  public ShinobiCard(String ID, CardStats info) {
    this(ID, info, getCardTextureString(removePrefix(ID), info.cardType));
  }

  public ShinobiCard(String ID, CardStats info, String cardImage) {
    this(
        ID,
        info.baseCost,
        info.cardType,
  ...
  protected final void setCustomVar(
      ...
      VariableType type,
      int base,
      TriFunction<ShinobiCard, AbstractMonster, Integer, Integer> preCalc) {
  ...
  protected final void setCustomVar(
      ...
      int base,
      TriFunction<ShinobiCard, AbstractMonster, Integer, Integer> preCalc,
      TriFunction<ShinobiCard, AbstractMonster, Integer, Integer> postCalc) {
  ...
  protected final void colorCustomVar(
      String key, Color normalColor, Color increasedColor, Color decreasedColor) {
  ...
  protected final void colorCustomVar(
    ...
    if (var == null) {
      throw new IllegalArgumentException(
          "Attempted to set color of variable that hasn't been registered.");
    }
  ...
  protected void setVarCalculation(
      String key, TriFunction<ShinobiCard, AbstractMonster, Integer, Integer> calculation) {
  ...
  public int customVar(String key) {
    LocalVarInfo var =
        cardVariables == null
            ? null
            : cardVariables.get(key); // Prevents crashing when used with dynamic text
    if (var == null) return -1;
    return var.value;
  }
  ...
  public AbstractCard makeStatEquivalentCopy() {
    ...
    if (card instanceof ShinobiCard) {
      card.rawDescription = this.rawDescription;
      ((ShinobiCard) card).upgradesDescription = this.upgradesDescription;

      ((ShinobiCard) card).baseCost = this.baseCost;

      ((ShinobiCard) card).upgradeCost = this.upgradeCost;
      ((ShinobiCard) card).upgradeDamage = this.upgradeDamage;
      ((ShinobiCard) card).upgradeBlock = this.upgradeBlock;
      ((ShinobiCard) card).upgradeMagic = this.upgradeMagic;

      ((ShinobiCard) card).costUpgrade = this.costUpgrade;
      ((ShinobiCard) card).damageUpgrade = this.damageUpgrade;
      ((ShinobiCard) card).blockUpgrade = this.blockUpgrade;
      ((ShinobiCard) card).magicUpgrade = this.magicUpgrade;

      ((ShinobiCard) card).baseExhaust = this.baseExhaust;
      ((ShinobiCard) card).upgExhaust = this.upgExhaust;
      ((ShinobiCard) card).baseEthereal = this.baseEthereal;
      ((ShinobiCard) card).upgEthereal = this.upgEthereal;
      ((ShinobiCard) card).baseInnate = this.baseInnate;
      ((ShinobiCard) card).upgInnate = this.upgInnate;
      ((ShinobiCard) card).baseRetain = this.baseRetain;
      ((ShinobiCard) card).upgRetain = this.upgRetain;

      for (Map.Entry<String, LocalVarInfo> varEntry : cardVariables.entrySet()) {
        LocalVarInfo target = ((ShinobiCard) card).getCustomVar(varEntry.getKey()),
            current = varEntry.getValue();
        if (target == null) {
          ((ShinobiCard) card).setCustomVar(varEntry.getKey(), current.base, current.upgrade);
          target = ((ShinobiCard) card).getCustomVar(varEntry.getKey());
        }
    ...
    if (!upgraded) {
      ...
      if (this.upgradesDescription) {
        if (cardStrings.UPGRADE_DESCRIPTION == null) {
          ShinobiMod.logger.error(
              "Card " + cardID + " upgrades description and has null upgrade description.");
        } else {
  ...
  private static class QuickDynamicVariable extends DynamicVariable {
    ...
    private ShinobiCard current = null;
    ...
    public void setIsModified(AbstractCard c, boolean v) {
      if (c instanceof ShinobiCard) {
        LocalVarInfo var = ((ShinobiCard) c).getCustomVar(localKey);
    ...
    public boolean isModified(AbstractCard c) {
      return c instanceof ShinobiCard && (current = (ShinobiCard) c).isCustomVarModified(localKey);
    }

    @Override
    public int value(AbstractCard c) {
      return c instanceof ShinobiCard ? ((ShinobiCard) c).customVar(localKey) : 0;
    }

    @Override
    public int baseValue(AbstractCard c) {
      return c instanceof ShinobiCard ? ((ShinobiCard) c).customVarBase(localKey) : 0;
    }

    @Override
    public boolean upgraded(AbstractCard c) {
      return c instanceof ShinobiCard && ((ShinobiCard) c).customVarUpgraded(localKey);
    }
  ...
  protected static class LocalVarInfo {
    ...
    TriFunction<ShinobiCard, AbstractMonster, Integer, Integer> calculation = LocalVarInfo::noCalc;
    ...
    private static int noCalc(ShinobiCard c, AbstractMonster m, int base) {
      return base;
    }
