package theshinobi.character;

import static theshinobi.ShinobiMod.characterPath;
...
public class MyCharacter extends CustomPlayer {
  ...
  public static final int STARTING_GOLD = 75;
  public static final int CARD_DRAW = 5;
  public static final int ORB_SLOTS = 0;

  // Strings
  private static final String ID =
      makeID("Shinobi"); // This should match whatever you have in the CharacterStrings.json file
  ...
  // This static class is necessary to avoid certain quirks of Java classloading when registering
  // the character.
  public static class Meta {
    // These are used to identify your character, as well as your character's card color.
    // Library color is basically the same as card color, but you need both because that's how the
    // game was made.
    @SpireEnum public static PlayerClass THE_SHINOBI;

    @SpireEnum(
        name = "CHARACTER_SHINOBI_COLOR") // These two MUST match. Change it to something unique for
    // your character.
    public static AbstractCard.CardColor CARD_COLOR;

    @SpireEnum(name = "CHARACTER_SHINOBI_COLOR")
    ...
    // This is used to color *some* images, but NOT the actual cards. For that, edit the images in
    // the cardback folder!
  ...
  // In-game images
  private static final String SHOULDER_1 =
      characterPath("shoulder.png"); // Shoulder 1 and 2 are used at rest sites.
  ...
  public MyCharacter() {
    ...
    initializeClass(
        ...
        -20.0F,
        200.0F,
        250.0F, // Character hitbox. x y position, then width and height.
        new EnergyManager(ENERGY_PER_TURN));

    // Location for text bubbles. You can adjust it as necessary later. For most characters, these
    // values are fine.
  ...
  private final Color cardRenderColor =
      Color.LIGHT_GRAY.cpy(); // Used for some vfx on moving cards (sometimes) (maybe)
  private final Color cardTrailColor =
      Color.LIGHT_GRAY.cpy(); // Used for card trail vfx during gameplay.
  private final Color slashAttackColor =
      Color.LIGHT_GRAY.cpy(); // Used for a screen tint effect when you attack the heart.
  ...
  public void doCharSelectScreenSelectEffect() {
    ...
    // See SoundMaster for a full list of existing sound effects, or look at BaseMod's wiki for
    // adding custom audio.
    CardCrawlGame.sound.playA("ATTACK_DAGGER_2", MathUtils.random(-0.2F, 0.2F));
    CardCrawlGame.screenShake.shake(
        ScreenShake.ShakeIntensity.MED, ScreenShake.ShakeDur.SHORT, false);
  }
  ...
  public String getVampireText() {
    return getText()[
        2]; // Generally, the only difference in this text is how the vampires refer to the player.
  }
